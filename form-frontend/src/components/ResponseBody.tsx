...
import { question } from '@/pages/fill/[...id]';
import { formElementObj, formObj } from '@/pages/index';
import ResponseElement from '@/components/ResponseElement';
import { subElementObj } from '@/pages/index';
const Responses = (props: { formState: formObj }) => {
  ...
  useEffect(() => {
    const getResponses = async () => {
      ...
      const res = await fetch(location.protocol + "//" + window.location.hostname + ":3003/forms/" + props.formState.id + "/submissions"
        , options);
