...
const FormLiveCheckbox = (props: { isLive: boolean, changeLive: any, mobile: boolean, formState: formObj }) => {
    ...
    const renderLiveLink=()=>{
        ...
        if(renderLive){
            return   <div className={` ${styles.liveLink} ${props.mobile ? '' : ''} `} id={`${props.isLive ? '' : styles.hidden}`}  >Form is now live at: <a href={window.location.protocol + '/fill/' + props.formState.id} target='_top'>{window.location.protocol + window.location.host + '/fill/' + props.formState.id}</a></div>
        }
    }
    return (
        <section className={`${props.mobile ? styles.mobileDisplay : ''} `}>
            <div className={`${styles.liveCheckWrapper} ${props.mobile ? '' : ''} `}>
