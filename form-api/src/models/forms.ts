...
export class formsStore {
    ...
    async updateForm(form: form): Promise<form> {
        try {
            ...
            const conn = await client.connect();//UPDATE comments SET votes = votes + ($1) WHERE id=($2) RETURNING *
            const sql = 'UPDATE forms SET title=($1), form=($2) WHERE id=($3) RETURNING *';
            //without json.stringify, JSON is stored with \" instead of "
            const results = await conn.query(sql, [form.title, JSON.stringify(form.form), form.id]);
            conn.release();
